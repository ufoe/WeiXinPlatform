...
<META name=format-detection content=telephone=no><LINK rel=stylesheet type=text/css href="<?php echo RES;?>/w54/w56.css">
...
<?php if(is_array($flash)): $i = 0; $__LIST__ = $flash;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$so): $mod = ($i % 2 );++$i;?><LI>
  <P><?php echo ($so["name"]); ?></P><IMG src="<?php echo ($so["img"]); ?>"> </LI><?php endforeach; endif; else: echo "" ;endif; ?></UL></DIV></DIV>
<DIV id=nav>
<DIV id=prev onClick="myScroll.scrollToPage('prev', 0,400,2);return false">← 
prev </DIV>
<UL id=indicator>
	<?php if(is_array($flash)): $i = 0; $__LIST__ = $flash;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$so): $mod = ($i % 2 );++$i;?><LI><?php echo ($i); endforeach; endif; else: echo "" ;endif; ?>
  </UL>
<DIV id=next onClick="myScroll.scrollToPage('next', 0);return false">next → 
</DIV></DIV>
<DIV class=clr></DIV></DIV>
<UL class=cateul>
	<?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($i < '4'): ?><LI class=li1><A 
  ...
  href="<?php if($vo['url'] == ''): echo U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token'])); else: echo (htmlspecialchars_decode($vo["url"])); endif; ?>">
  <DIV class=menubtn>
  <DIV class=menutitle><?php echo ($vo["name"]); ?> </DIV></DIV></A></LI><?php endif; endforeach; endif; else: echo "" ;endif; ?>
  </UL>
  <script>
