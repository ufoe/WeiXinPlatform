...
class UpyunAction extends UserAction{
	...
	public function upload(){
		...
		$options['expiration'] = time()+3600*24*3650; /// 授权过期时间
	...
	public function uploadReturn(){		
			...
			$host = 'bcs.duapp.com';
			$ak = C('up_form_api_key');
			$sk = C('up_form_api_secret');
			...
			if(!in_array(substr($ext,1),explode(',', C('up_exts')))){

				$message='图片格式不正确!';
			}else{				
				$object =  '/'.date('Y-m-d').'/'.time () . "-" . rand ( 1000, 999 ).$ext;
	...
	function kindedtiropic(){
		...
		try{
			if (empty($_FILES) === false) {
				...
				$ext = strrchr($_FILES['imgFile']['name'], '.');		
				...
				if(!in_array(substr($ext,1),explode(',', C('up_exts')))){
					...
					$object =  '/'.$dir_name.'/'.date('Y-m-d').'/'.time () . "-" . rand ( 1000, 999 ).$ext;
					...
					if ($response->isOK ()) {
						...
						echo json_encode(array('error' => 0, 'url' => $hd['_info']['url']));
